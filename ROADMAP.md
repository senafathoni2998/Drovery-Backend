...
9. **Notification preferences** ✅ **Done (part 1)** + a real **support chat** *(still open)*. `GET/PATCH /notifications/preferences` exposes per-user `pushEnabled`, `deliveryUpdates`, `promotions`, and `quietHoursStart/End`. The in-app feed is **always** recorded; preferences gate only the **push** fan-out. Quiet hours are `[start,end)` wall-clock hours (wrap-around handled), evaluated in `NOTIFICATIONS_TZ` (default `Asia/Jakarta`, not the UTC container) and require both bounds together. **Still open:** the real-time **support chat** (the "Live Chat" button is a no-op; ticket persistence backs it — the natural next build is chat over the existing WebSocket tier).
