...
- **L2 — horizontally shard the primary (when L1 is exhausted).** A thin `ShardRouter` **above**
  ...
  **Phase-3 decoupling — the vetted plan (Opus design panel: debit-first-saga vs reserve-then-settle vs user-home-shard).**
  ...
  - **STAGED (each additive, default-OFF, own crash-permutation specs):** **A1 ✅ SHIPPED** — pre-generate `deliveryId` in `create()` and thread it into `deliveryData.id` (byte-identical; makes every money key knowable up-front; minted ONCE so it survives the trackingId-retry loop without a second debit; `createdAt` stays DB-authoritative). **A2 (next)** — the reorder behind `DELIVERY_DEBIT_FIRST` (pull redeem+debit into pre-delivery user-shard txns; REMOVE them from the delivery-tx loop; compute `finalTotal` after; in-process compensation catch). **A3** — a worker-tier reconciliation **janitor** (no migration): compensate orphaned `CHECKOUT_SPEND` debits / `REDEEMED` promos whose `deliveryId` has no `Delivery` row older than a **grace window** (the window is itself a money knob — too short re-introduces under-charge).
