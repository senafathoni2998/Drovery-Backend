...
| **🟡 Transactional outbox — Stage-1 SHIPPED** (§2) | The DB-write-shard unblock has begun. `create()`'s `$transaction` co-commits the delivery with **user-rooted wallet/promo/referral** mutations — a single Prisma tx can't span shards, so they must move to an outbox first. Stage-1 ships the outbox machinery (`OutboxEvent` table + `OutboxService.enqueueWithinTx` + a worker-tier dispatcher: atomic PENDING→PROCESSING CAS claim, co-committed grant+PROCESSED, P2002-as-success, stale-claim reaper, FAILED-after-N replay, metrics) and decouples the **referral reward** — a pure credit with no failure mode — behind `DELIVERY_OUTBOX_REFERRAL` (default OFF = today's inline grant, byte-identical; the dispatcher re-runs the canonical `maybeGrantReferralRewardWithinTx` CAS so behavior is identical). | Stages 2-3: move **wallet-debit** + **promo** (they have insufficient-funds/over-redeem failure modes → need a reservation/compensation design) — THEN the `create()` tx is single-shard and the `ShardRouter` can flip `shardCount>1` |
...
- **L2 — horizontally shard the primary (when L1 is exhausted).** A thin `ShardRouter` **above**
  ...
  > `shardCount>1` corrupts balances. This refactor — not the router code — is the real Phase-3 work.

  **Phase-3 decoupling — the vetted plan (Opus design panel: debit-first-saga vs reserve-then-settle vs user-home-shard).**
  - ✅ **Referral** — SHIPPED via the Stage-1 outbox (pure credit, no failure mode).
  - 🟡 **Wallet-debit + promo** (both *charge-gating* — they reduce the card total, so they CANNOT be deferred eventually-consistent like the referral): **WINNER = the debit-first saga.** Pre-generate the delivery id; run the authoritative `wallet.debit` + `promo.redeem` in their OWN single-shard (user-shard) `$transaction`s FIRST, keyed to that id; compute `finalTotal` from the **confirmed** reservations; then create the delivery in a SEPARATE single-shard (delivery-shard) tx. Feasible because `WalletTransaction.deliveryId` / `PromoRedemption.deliveryId` carry **no FK** to `deliveries` (reserve-first rows insert with no delivery row yet). Compensate orphans with the EXISTING idempotent `refundForDelivery` / `releaseForDelivery`. Rejected: *reserve-then-settle* (same guarantee, strictly more state + a TTL that is load-bearing for **money**, and its auto-release-vs-settle race can UNDER-charge); *user-home-shard* (solves the wallet axis with zero saga, but trades even load for **hot-shard skew** and still leaves the global `promoCode.timesRedeemed` counter cross-shard for capped codes).
  - **STAGED (each additive, default-OFF, own crash-permutation specs) — ✅ ALL SHIPPED:** **A1** — pre-generate `deliveryId` in `create()`, threaded into `deliveryData.id` (byte-identical; every money key knowable up-front; minted ONCE so it survives the trackingId-retry loop; `createdAt` stays DB-authoritative). **A2** — the reorder behind `DELIVERY_DEBIT_FIRST` (redeem+debit run as authoritative reservations in pre-delivery user-shard txns; skipped in the delivery-tx loop; `finalTotal` backed by committed reservations; in-process compensation on every failure, incl. the commit-ambiguous debit). **A3** — the worker-tier reconciliation **janitor** (`src/deliveries/orphan-reaper/`, no migration): a sliding-window sweep that re-checks delivery existence and reverses orphaned `CHECKOUT_SPEND` debits / `REDEEMED` promos (idempotent), inert unless `DELIVERY_DEBIT_FIRST` is on. **Grace window** = the load-bearing money knob (must exceed worst-case delivery-tx + retry latency, or it refunds a reservation whose delivery is about to commit). With A1-A3 the wallet/promo writes leave the cross-shard tx; the **only** remaining cross-shard write is the global `promoCode.timesRedeemed` counter for CAPPED codes (per-shard replicate+reconcile, or accept bounded over-redeem) + the `trackingId` registry (shard-prefix the public id).
  - **KILLER RISKS to design against in A2/A3:** the orphan grace window (must exceed worst-case delivery-tx + full trackingId-retry latency); the janitor is correctness-not-cleanup (worker-tier + alerting); the delivery id must be minted INSIDE `create()` only (a reused id → debit P2002 → silent lost-debit); A2 must physically remove redeem/debit from the retry-loop body (else a collision re-runs them → P2002 mis-handled); compensation-on-throw must be unconditional + idempotent; flag-OFF stays byte-identical.
...
- **`create()` cross-shard tx:** outbox/saga (correct, adds eventual consistency + a compensating-refund
  ...
  window is acceptable? — **DECIDED: outbox/saga. Stage-1 shipped** (machinery + the referral reward,
  the no-failure-mode effect, behind `DELIVERY_OUTBOX_REFERRAL`); wallet-debit + promo (which DO have
  failure modes) await a reservation/compensation design before they can move and the tx goes single-shard.
