...
describe('SupportController', () => {
  ...
  beforeEach(async () => {
    supportService = {
      ...
      submitTicket: jest
        .fn()
        .mockReturnValue({ success: true, ticketId: 'ticket-1' }),
  ...
  describe('submitTicket', () => {
    it('should delegate to supportService.submitTicket (trimmed)', () => {
      const result = controller.submitTicket('user-1', {
        message: '  Help me  ',
      });

      expect(supportService.submitTicket).toHaveBeenCalledWith(
        'user-1',
        'Help me',
      );
  ...
  describe('getMessages', () => {
    it('delegates to chat.getMessages with paging defaults', () => {
      ...
      expect(chat.getMessages).toHaveBeenCalledWith(
        'user-1',
        'ticket-1',
        50,
        0,
  ...
  describe('sendMessage', () => {
    it('persists then publishes the message and returns the wire payload', async () => {
      ...
      expect(chat.createUserMessage).toHaveBeenCalledWith(
        'user-1',
        'ticket-1',
        'hello',
      );
