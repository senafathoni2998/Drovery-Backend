...
| Wallet | `GET /wallet` | jwt | `{balance, currency, transactions[]}` — credit balance + paginated ledger. Spend at checkout with `useCredits:true` on `POST /deliveries` (stacked after promo) |
| Referrals | `GET /referrals` | jwt | `{referralCode, rewardPerReferral, stats, referrals[]}`. Signup accepts an optional `referralCode`; the referee's first delivery credits both parties |
...
| **Admin / agent** (role-gated, `/admin/*`) | support inbox `GET /admin/support/tickets`, `GET/POST .../{id}/messages`, `PATCH .../{id}/status` (AGENT+ADMIN); `GET /admin/deliveries[/{id}]`, `POST .../{id}/force-cancel`, `POST .../{id}/refund`; `GET/POST /admin/promos`, `GET/PATCH /admin/promos/{id}`; `GET /admin/overview`; `GET /admin/users`, `PATCH /admin/users/{id}/role` (ADMIN) | jwt + role | operator + support-agent API. Requires `role` AGENT/ADMIN (resolved fresh from the DB; non-admins get 403). Seeded admin: `admin@drovery.com`. **Internal — a separate `drovery-admin` web UI consumes this; not the consumer app** |
