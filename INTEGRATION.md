...
On create, the backend geocodes the addresses (if coords weren't supplied), then spawns an **in-memory `setTimeout` simulation** that auto-advances status (`CONFIRMED@10s, DRONE_ASSIGNED@25s, PICKUP@45s, IN_TRANSIT@70s, DELIVERED@120s`), **interpolates the drone position every 5s** along the route, upserts `DeliveryTracking`, and writes + pushes a `Notification` on each transition.
